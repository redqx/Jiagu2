...
{
    void *generic_stub;
    void *(*quick_on_stack_replace)(
            const void *param1,
            const void *param2,
            const void *fake_trampoline,
            const void *called);
} STUBS;

void JNIEXPORT ndk_init(JNIEnv *env)
{
    if (SDK_INT <= 0)//一开始结果就是0
    {
        char sdk[PROP_VALUE_MAX];
        __system_property_get("ro.build.version.sdk", sdk);//系统属性获取
        SDK_INT = atoi(sdk);
        LOGD("SDK_INT = %d", SDK_INT);
        if (SDK_INT >= 24)//Android7.0分界线
        {
            static __attribute__((__aligned__(PAGE_SIZE))) uint8_t __insns[PAGE_SIZE];
            STUBS.generic_stub = __insns;
            mprotect(__insns, sizeof(__insns), PROT_READ | PROT_WRITE | PROT_EXEC);//设置一块rwx的内存
            ...
            /*
                        ...
                        */
                        memcpy(quick_on_stack_back,
                               "\xC3\xFF\x74\x24\x08\xC3",
                               6);
            ...
            # error "not supported"
            #endif
            LOGD("init done! quick_on_stack_replace = %p, quick_on_stack_back = %p",STUBS.generic_stub, quick_on_stack_back);
        } //if
    } //if
}
